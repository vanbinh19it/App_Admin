  ...
  const fetchVocabularies = async (): Promise<void> => {
    try {
      const response = await axios.get<VocabularyData[]>(
        "http://192.168.1.157:3005/api/vocabularies"
      );
      ...
      [
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              await axios.delete(
                `http://192.168.1.157:3005/api/vocabularies/${id}`
              );
  ...
  const handleSubmit = async (): Promise<void> => {
    ...
    try {
      if (editItem) {
        ...
        await axios.put(
          `http://192.168.1.157:3005/api/vocabularies/${editItem._id}`,
          {
            category: selectedCategory,
            item: {
              ...formData,
        ...
        await axios.post("http://192.168.1.157:3005/api/vocabularies", {
          category: selectedCategory,
          item: formData,
        });
  ...
  const renderVocabularyItem = ({ item }: { item: VocabularyItem }) => (
    <View style={styles.itemContainer}>
      <View style={styles.itemContent}>
        <Image
          source={{ uri: `http://192.168.1.157:3005${item.image}` }}
